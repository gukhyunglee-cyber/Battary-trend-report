...
1. **핵심 이슈 4가지를 선정**하세요 (전주 대비 새로 등장한 주제를 우선 고려)
   - 반드시 다음 주제들을 최대한 찾아 반영하거나 집중 조명하세요:
...
요구사항:
1. 핵심 이슈 4가지를 선정하여 각각 1개 슬라이드로 구성 (전주 대비 새로운 이슈 우선)
   - 이슈 선정 시 다음 4가지 주제를 강력하게 반영하세요: (배터리 제조공정 제조사별 비교 분석, 신규 제조 설비 및 신공법, 국내/중국 설비업체 최신 동향, 배터리 전문 전시회 출품 기술)
2. 마지막 슬라이드는 "전주 대비 변화 하이라이트"로 구성
3. 각 슬라이드에는 5~7개의 상세 불릿포인트 포함 (문장 형태)
4. 각 슬라이드에 관련된 소스 URL을 1개 이상 포함
...
이메일 구조:
1. 정중한 인사말 (작성일: {today})
2. 핵심 이슈 전부를 요약 (각 이슈별 관련 원본 소스 URL 링크 반드시 포함)
3. 전주 대비 새로운 소식 추가 (전주와 달라지거나 새롭게 부상한 이슈 리스업)
4. 맺음말로 결론
